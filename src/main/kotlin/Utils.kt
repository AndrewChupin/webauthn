...
fun String.fromHex() = hexStringToByteArray(this)

fun ByteArray.toHex(): String =
    toHexString(this, 0, size, false)
...
fun cleanHexPrefix(input: String): String = if (containsHexPrefix(input)) {
    input.substring(2)
} else {
    input
}
...
fun hexStringToByteArray(input: String): ByteArray {
    ...
    var i = startIdx
    while (i < len) {
        data[(i + 1) / 2] = ((Character.digit(cleanInput[i], 16) shl 4) + Character.digit(
            cleanInput[i + 1],
            16
        )).toByte()
        i += 2
